...
INSERT INTO 
    ...
    SELECT
        ...
        'Laermemfindlichkeitsstufen' AS subthema,
        substring(typ_ueberlagernd_flaeche.typ_kt FROM 1 FOR 4) AS artcode,
        'urn:fdc:ilismeta.interlis.ch:2017:NP_Typ_Laermemfindlichkeitsstufen' AS artcodeliste,      /* TODO: woher stammt die artcodeliste?' */
        CASE 
            WHEN ueberlagernd_flaeche.rechtsstatus IS NULL THEN 'inKraft' /* TODO: tbd */
            ELSE ueberlagernd_flaeche.rechtsstatus
        END AS rechtsstatus,
        ueberlagernd_flaeche.publiziertab, /* TODO: tbd */
...
--     AND
--     typ_ueberlagernd_flaeche.t_id <> 124                /* PS Datenfehler???? */
;
...
(
    ...
    FROM
        ...
        ON typ_dokument.typ_ueberlagernd_flaeche = eigentumsbeschraenkung.t_id,
        basket_dataset
    WHERE
...
--     WHERE                                                                      /* PS stimmt das so?, die Dokumente sollen nur 1x importiert werden */
...
INSERT INTO
    ...
    SELECT
        ...
        basket_dataset.basket_t_id AS t_basket,
        basket_dataset.datasetname AS t_datasetname,
        ST_MakeValid(ST_RemoveRepeatedPoints(ST_SnapToGrid(ueberlagernd_flaeche.geometrie, 0.001))) AS flaeche_lv95,
        ueberlagernd_flaeche.rechtsstatus AS rechtsstatus,
        ueberlagernd_flaeche.publiziertab AS publiziertab,
    ...
    FROM
        ...
        RIGHT JOIN agi_oereb_npl_staging.transferstruktur_eigentumsbeschraenkung AS eigentumsbeschraenkung
        ON ueberlagernd_flaeche.typ_ueberlagernd_flaeche = eigentumsbeschraenkung.t_id,
        (
            SELECT
                basket.t_id AS basket_t_id,
                dataset.datasetname AS datasetname               
...
--     WHERE
--         ueberlagernd_flaeche.t_id IS NOT NULL                                              /* PS stimmt das so?*/
;
...
WHERE
    subthema = 'Laermemfindlichkeitsstufen'
;
